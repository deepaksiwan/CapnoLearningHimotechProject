...
const ChartTable = () => {
    ...
    return (
        <div>
            <ChartExportedHeader config={config} />
            <div className="wrp-charttable">
                <div className="container-fluid">
                    <div className="row">
                        {
                             graphs && graphs.map(function (d, i) {
                                console.log(d);
                                return (
                                    <div className="chart-w" style={{ width:  (eval(d.col) * 100) + "%" , maxWidth: (eval(d.col) * 100) + "%", height: (eval(d.row) * 84) + "vh" }}>
                                        <ExportChart record={record} session={session} signal={d.signal_name} xmax={d.xmax} xmin={d.ymin}  ymin={d.ymin} ymax={d.ymax}  type={d.type} color={d.color} />
                                        </div>
