...
const Subscriptionmanagement = () =>{
    ...
    const subscriptionSave = () => {
        ...
        fetch(API_URL+"/subscription/update/" + userId, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'x-access-token': accessToken
                   ...
                   <div className="software-updt-child1">
                       {
                           ...
                           <div className="radio-input"><input class="form-check-input" onChange={() => handleAutoUpdate(0)} type="radio" defaultChecked={owner.autoupdate == 0? true : false} value="0" name="autoaupdate" /><span>{t('no')}</span></div>
                           <div className="radio-input"> <input class="form-check-input" name="autoaupdate" type="radio" defaultChecked={owner.autoupdate == 1? true : false} value="1" onChange={() => handleAutoUpdate(1)} /><span>{t('yes')}</span></div>
